...
var multilanguage_inline_default = 'var a=window.__quartzMultilanguage,h;function y(){return document.body?.dataset?.basepath??""}function L(e){let t=e.replace(/(^|\\/)index$/,"$1");return`${y()}/${t}`}function m(e){if(!a||!e)return;let t=e.trim().toLowerCase(),n=t.split(/[-_]/)[0];return(a.languages.find(i=>i.code===t)??a.languages.find(i=>i.locale.toLowerCase()===t)??a.languages.find(i=>i.code===n))?.code}function E(){if(a?.rememberChoice)try{return m(localStorage.getItem(a.storageKey))}catch{return}}function w(){let e=E();if(e)return e;let t=navigator.languages?.length?navigator.languages:[navigator.language];for(let n of t){let o=m(n);if(o)return o}}function p(e){return a?.languages.find(t=>t.code===e)?.native??e.toUpperCase()}function v(e){return a?.languages.find(t=>t.code===e)?.locale??e}function b(){return document.body?.dataset?.slug??""}function C(){let e=document.querySelector(".multilanguage-switcher[data-lang]"),t=document.querySelector(".multilanguage-notice[data-ml-lang]");return e?.dataset.lang??t?.dataset.mlLang??m(document.documentElement.lang)??a?.defaultLanguage}async function S(e){let t=document.querySelector(".multilanguage-switcher[data-translations]");if(t?.dataset.translations)try{return JSON.parse(t.dataset.translations)}catch{}return a?(h??(h=fetch(`${y()}/${a.dataPath}`).then(o=>o.ok?o.json():void 0).catch(()=>{})),(await h)?.pages[e]?.translations??{}):{}}function k(){if(!a?.rememberChoice)return;let e=document.querySelectorAll(".multilanguage-switcher a[data-lang]");for(let t of e){let n=()=>{try{localStorage.setItem(a.storageKey,t.dataset.lang??"")}catch{}};t.addEventListener("click",n),window.addCleanup(()=>t.removeEventListener("click",n))}}function T(){let e=document.querySelectorAll("details.multilanguage-switcher");if(e.length===0)return;let t=i=>{for(let s of e)s.open&&!(i instanceof Node&&s.contains(i))&&(s.open=!1)},n=i=>t(i.target),o=i=>{i.key==="Escape"&&t()};document.addEventListener("click",n),document.addEventListener("keydown",o),window.addCleanup(()=>document.removeEventListener("click",n)),window.addCleanup(()=>document.removeEventListener("keydown",o))}async function M(){if(!a||!(a.noticeMissing||a.noticeAvailable))return;let e=document.querySelector(".multilanguage-notice");if(!e)return;e.hidden=!0;let t=C(),n=w();if(!t||!n||n===t)return;let o=a.notices?.[n];if(!o)return;let i=e.dataset.mlSlug??b(),s=await S(i),u=s[n];if(u?!a.noticeAvailable:!a.noticeMissing)return;let f=e.querySelector(".callout-content")??e;f.replaceChildren();let l=document.createElement("p");if(l.lang=v(n),u){let[d="",g=""]=o.available.split("{{}}"),r=document.createElement("a");r.className="internal",r.dataset.mlLink="",r.href=L(u),r.hreflang=n;let c=document.createElement("strong");c.textContent=p(n),r.append(c),l.append(d,r,g)}else{let[d="",g=""]=o.missing.split("{{}}"),r=Object.keys(s).length>0?Object.keys(s):[t],c=document.createElement("span");c.dataset.mlLanguages="",c.textContent=r.map(p).join(", "),l.append(d,c,g)}f.append(l),e.hidden=!1}function D(){if(!a?.localizeDates)return;let e=document.documentElement.lang||a.defaultLanguage,t;try{t=new Intl.DateTimeFormat(e,{year:"numeric",month:"short",day:"2-digit"})}catch{return}for(let n of document.querySelectorAll("time[datetime]")){if(n.dataset.mlLocalized===e)continue;let o=new Date(n.dateTime);Number.isNaN(o.getTime())||(n.textContent=t.format(o),n.dataset.mlLocalized=e)}}function q(){let e=document.querySelector(".multilanguage-switcher[data-lang]"),t=document.querySelector(".multilanguage-notice[data-ml-lang]"),n=e?.dataset.lang??t?.dataset.mlLang;if(!n)return;let o=v(n);document.documentElement.lang!==o&&(document.documentElement.lang=o)}function H(){a&&(q(),k(),T(),D(),M())}document.addEventListener("nav",H);\n';
