...
/**
 * Sets the `lang` attribute of the `<html>` tag. Returns `undefined` when there is no `<html>`
 * tag or the attribute already has that value.
 */
export function withHtmlLang(html: string, locale: string): string | undefined {
  const match = /<html\b[^>]*>/i.exec(html);
  if (!match) return undefined;
  const tag = match[0];
  const attr = ` lang="${escapeHTML(locale)}"`;
  const next = /\slang="[^"]*"/i.test(tag)
    ? tag.replace(/\slang="[^"]*"/i, attr)
    : tag.replace(/^<html/i, `<html${attr}`);
  if (next === tag) return undefined;
...
export const MultilanguageEmitter: QuartzEmitterPlugin<MultilanguageOptions> = (userOpts) => {
  ...
  /**
   * Quartz renders `<html lang>` from `frontmatter.lang`, which the transformer fills in for
   * Markdown pages only. Pages generated by page-type plugins (folder and tag pages) never pass
   * the transformer and fall back to the site locale, so `en/docs/index` came out as `lang="de"`.
   * No hook runs before those pages render, but the dispatcher writes them before any other
   * emitter runs (also in serve mode), so the attribute is corrected in the written file, using
   * the same detection as `og:locale`.
   */
  async function* fixGeneratedPageLang(ctx: BuildCtx): AsyncGenerator<FilePath> {
    const generated = (ctx as BuildCtx & { virtualPages?: ProcessedContent[] }).virtualPages ?? [];
    for (const [, file] of generated) {
      ...
      const frontmatter =
        data.frontmatter && typeof data.frontmatter === "object"
          ...
          : undefined;
      // A page type that sets a language itself knows better.
      if (typeof frontmatter?.lang === "string" && frontmatter.lang.trim()) continue;
      const { lang } = detectLanguage({ slug, frontmatter }, opts);
      const locale = findLanguage(opts, lang)?.locale ?? lang;
      const target = joinSegments(ctx.argv.output, `${slug}.html`) as FilePath;
      let html: string;
      try {
